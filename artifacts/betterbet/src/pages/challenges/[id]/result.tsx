...
import { Trophy, Home, Frown, PartyPopper } from "lucide-react";
...
export default function ChallengeResult() {
  ...
  if (isLoading || !summary) return <Layout><div className="p-8 text-center">Loading...</div></Layout>;
  ...
  const isTie = winner === "Tie" || winner === null;
    ...
    <Layout showNav={false}>
      <div className="flex-1 flex flex-col items-center justify-center p-4 py-12 text-center">
        ...
        <p className="text-xl font-medium text-muted-foreground mb-8">
          {outcome || (amIWinner ? "You dominated." : "Better luck next time.")}
        </p>

        <Card className={`w-full max-w-sm mb-8 border-2 shadow-xl ${amIWinner ? 'border-primary bg-primary/5' : isTie ? 'border-border' : 'border-destructive bg-destructive/5'}`}>
          <CardContent className="p-6">
            <div className="text-sm font-bold uppercase tracking-widest text-muted-foreground mb-2">The Settlement</div>
            {isTie ? (
        ...
        <div className="grid grid-cols-2 gap-4 w-full max-w-sm mb-12">
          ...
          <div className="bg-muted p-4 rounded-xl text-center">
            <p className="text-xs font-bold uppercase text-muted-foreground mb-1">{opponentProgress?.displayName}'s</p>
